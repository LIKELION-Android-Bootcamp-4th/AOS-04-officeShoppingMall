...
class OrderService {
  ...
  Future<List<OrderDTO>> getOrder() async {
    final response = await _dio.get(Api.order.getOrders());
  ...
  Future<void> addOrder(OrderDTO order) async {
    final response = await _dio.post(Api.order.addOrder());
  ...
  Future<void> cancelOrder(String id) async {
    final response = await _dio.patch(Api.order.cancelOrder(id));
    if (response.statusCode == 200) {
      print('주문 취소 성공!');
    } else {
      throw Exception(
